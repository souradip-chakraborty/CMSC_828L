...
class Trainer:
    ...
    def setup(self, training_data):
        ...
        self.parameters = {'hidden_layers' : self.n_layers}
        node_list = [self.n_neurons]*self.n_layers
        node_list[-1] = 1
        ...
        self.loss_layer = layers.CrossEntropy(self.network.MY_LAST_LAYER, y)
...
def main(test=False):
    ...
    if not test:
        ...
        #define the dict
        data_dict = data_generators.data_4b()
        ...
        _, _, _, _ = trainer.setup(training_data)
        ...
        test_loss = layers.CrossEntropy(trainer.network.MY_LAST_LAYER, y_test).forward()
